...
namespace Aspose.ThreeD.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeThreeDCloudContext.atcc = new AsposeThreeDCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeThreeDCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeThreeDCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
